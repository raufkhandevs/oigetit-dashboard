...
export const SentimentFilterContext = createContext<
  SentimentFilterContextType | undefined
>(undefined);
...
export const useSentimentFilter = () => {
  ...
  if (!context) {
    throw new Error(
      'useSentimentFilter must be used within a SentimentFilterProvider'
    );
  }
  return context;
};
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      const savedSentimentFilter = localStorage.getItem(
        'activeSentimentFilter'
      );
      if (
        savedSentimentFilter &&
        ['all', 'positive', 'neutral', 'negative'].includes(
          savedSentimentFilter
        )
      ) {
  ...
  return (
    <SidebarProvider>
      <TimeRangeContext.Provider
        value={{ activeTimeRange, setActiveTimeRange }}
      >
        <DateRangeContext.Provider
          value={{ customDateRange, setCustomDateRange, formattedDateRange }}
        >
          <HashtagContext.Provider value={{ activeHashtag, setActiveHashtag }}>
            <SentimentFilterContext.Provider
              value={{ activeSentimentFilter, setActiveSentimentFilter }}
            >
                ...
                <div className='flex flex-col h-screen w-full'>
                  ...
                  <div className='flex flex-1 overflow-hidden'>
                    ...
                    <div className='flex-1 flex flex-col overflow-auto bg-[#F5F5F6]'>
                      ...
                      <div className='flex-1 p-2 bg-[#F5F5F6]'>{children}</div>
                    </div>
                  </div>
                </div>
              </MobileMenuContext.Provider>
