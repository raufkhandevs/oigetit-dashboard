...
export function KeyMetricsPage() {
  ...
  const [resultsOverTimeData, setResultsOverTimeData] = useState<
    ChartDataPoint[]
  >([]);
  ...
  useEffect(() => {
    if (apiHistogramData && Array.isArray(apiHistogramData)) {
      ...
      const resultData: ChartDataPoint[] = apiHistogramData.map((point) => ({
        date: new Date(point.pubdate).toLocaleDateString('en-US', {
          day: 'numeric',
          month: 'short',
        }),
        value: point.volume,
      ...
      const totalResults = apiHistogramData.reduce(
        (sum, point) => sum + point.volume,
        0
      );
      const totalEngagement = apiHistogramData.reduce(
        (sum, point) => sum + point.volume * 5,
        0
      ); // Example calculation

      const totalPositive = apiHistogramData.reduce(
        (sum, point) => sum + point.volume_pos,
        0
      );
      const totalNegative = apiHistogramData.reduce(
        (sum, point) => sum + point.volume_neg,
        0
      );
      const totalVolume = apiHistogramData.reduce(
        (sum, point) => sum + point.volume,
        0
      );

      const positiveSentiment =
        totalVolume > 0 ? (totalPositive / totalVolume) * 100 : 0;
      const negativeSentiment =
        totalVolume > 0 ? (totalNegative / totalVolume) * 100 : 0;
  ...
  if (error) {
    return (
      <div className='p-6 text-red-500'>Error loading key metrics data</div>
    );
  }
  ...
  return (
    <div className='space-y-4 p-6 bg-gray-100'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4 '>
        ...
        <ChartCard
          title={getChartTitle('Result Over Time')}
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
        <div className='flex flex-col gap-4 justify-between'>
          <MetricCard
            ...
            value={
              <SentimentDisplay
                positive={metrics.positiveSentiment}
                negative={metrics.negativeSentiment}
              />
            }
          />
          <MetricCard
        ...
        <ChartCard
          title={getChartTitle('Net Sentiment Over Time')}
          className='md:col-span-2'
        >
          <SentimentChart
